...
class Categorical_Topk(nn.Module):
    ...
    def forward(self, x, available_actions=None):
        ...
        # P0-B: exclude self from collaborator candidates
        diag_mask = 1.0 - torch.eye(x.size(0), x.size(1), device=x.device)
        x = x * diag_mask + (1.0 - diag_mask) * (-1e10)
    ...
    def log_probs(self, actions):
        eps = torch.finfo(self.probs.dtype).eps
        return torch.gather(torch.log(self.probs.clamp_min(eps)), -1, actions.long())
