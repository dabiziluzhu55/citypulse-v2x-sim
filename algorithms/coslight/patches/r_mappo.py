...
class R_MAPPO_Trans():
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        adv_targ, available_actions_batch, score_batch, old_score_log_probs_batch, actor_features_batch, trans_masks_batch = sample
        ...
        if self.args.use_3cons:
            atten_scores = self.policy.actor.model.compute_score(obs_batch, trans_masks_batch)  ## 20.8.8

        # Reshape to do in a single forward pass for all steps
        
        values, action_log_probs, dist_entropy, score_action_log_probs, score_dist_entropy, score_action_logits = self.policy.evaluate_actions(share_obs_batch,
                                                                              obs_batch, 
                                                                              rnn_states_batch, 
        ...
        # actor update
        imp_weights = torch.exp(action_log_probs - old_action_log_probs_batch) # torch.Size([120, 8, 1])
        ...
        # group update
        score_imp_weights= torch.exp((score_action_log_probs - old_score_log_probs_batch).sum(-1, keepdim=True)) # torch.Size([120, 8, 5])
        score_surr1 = score_imp_weights * adv_targ
        score_surr2 = torch.clamp(score_imp_weights, 1.0 - self.clip_param, 1.0 + self.clip_param) * adv_targ
        if self._use_policy_active_masks:
            score_policy_action_loss = (-torch.sum(torch.min(score_surr1, score_surr2), dim=-1,
                                            keepdim=True) * active_masks_batch).sum() / active_masks_batch.sum()
        else:
            score_policy_action_loss = -torch.sum(torch.min(score_surr1, score_surr2), dim=-1, keepdim=True).mean()
        ...
        if self.args.use_3cons:
            # Attention is a guide distribution, not an optimization target.
            # Detach it so target-side log(0) derivatives cannot poison the
            # transformer that produced the guide. Masking/dropout can create
            # exact zeros, so clamp and renormalize both distributions.
            atten_scores_kl = atten_scores.detach()
            if not torch.isfinite(atten_scores_kl).all():
                raise FloatingPointError("non-finite attention scores before KL")
            eps = torch.finfo(atten_scores_kl.dtype).eps
            atten_scores_kl = atten_scores_kl.clamp_min(eps)
            atten_scores_kl = atten_scores_kl / atten_scores_kl.sum(
                dim=-1, keepdim=True
            ).clamp_min(eps)

            score = torch.softmax(score_action_logits, dim=-1)
            score = score.clamp_min(eps)
            score = score / score.sum(dim=-1, keepdim=True).clamp_min(eps)
            kl_loss = F.kl_div(score.log(), atten_scores_kl, reduction='batchmean')
            if not torch.isfinite(kl_loss):
                raise FloatingPointError("non-finite KL loss")
        ...
        if update_actor:
            (policy_loss + score_policy_action_loss - dist_entropy * self.entropy_coef - score_dist_entropy * self.entropy_coef).backward()
        ...
        if self.args.use_3cons:
            return value_losses, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, (kl_loss, score_policy_action_loss, diag_mean_loss, sym_loss, score_dist_entropy)
        else:
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                if not self.args.use_kl:
                    ...
                    value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, (kl_loss, group_loss, diag_mean_loss, sym_loss, score_dist_entropy)\
                        = self.ppo_update(sample, update_actor)
