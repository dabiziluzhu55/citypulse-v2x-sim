...
class TSCSimulator:
    def __init__(self, config, port, not_default=True, output_path=None):
        ...
        self.is_libsumo = config.get('is_libsumo', False)
        ...
        if self.is_record:
            if not os.path.exists(self.output_path):
                try:
                    ...
                    pass
            output_stem = self.output_path + ('%s_%s_%s' % (
                self.name, sumocfg_file.split("/")[-1], self.p))
            command += ['--tripinfo-output', output_stem + '_trip.xml',
                        '--tripinfo-output.write-unfinished',
                        '--summary-output', output_stem + '_summary.xml',
                        '--error-log', output_stem + '_error.log']
        # subprocess.Popen(command)
        if self.is_libsumo:
            if libsumo is None:
                raise RuntimeError('libsumo requested but is unavailable')
        ...
        if self.action_type == 'select_phase':
            for tl, a in action.items():
                ...
                if a == current_phase_index:
                    self._crosses[tl].set_phase_by_index(a, self.iter_duration + self._yellow_duration)
                else:
