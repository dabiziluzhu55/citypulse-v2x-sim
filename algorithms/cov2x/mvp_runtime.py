...
from algorithms.cov2x.contract import (
    ...
    ROAD_FOCUSED_ACTOR_UPDATE_SCHEDULE_ID,
    ROAD_FOCUSED_ACTOR_UPDATES,
    ROAD_FOCUSED_CANDIDATE_ID,
    ROAD_FOCUSED_CHECKPOINT_FORMAT_VERSION,
    ROAD_FOCUSED_CRITIC_LINEAGE,
    ROAD_FOCUSED_FROZEN_ACTOR_ROLES,
    ROAD_FOCUSED_PARENT_CANDIDATE_ID,
    ROAD_FOCUSED_PARENT_CHECKPOINT_FORMAT_VERSION,
    ROAD_FOCUSED_PARENT_CHECKPOINT_GENERATION,
    ROAD_FOCUSED_PARENT_CHECKPOINT_SHA256,
    ROAD_FOCUSED_REWARD_SEMANTICS,
...
from algorithms.cov2x.reward_ledger import VehicleLifecycleLedger, network_time_loss_reward
...
from algorithms.cov2x.smdp import SMDPTransition, assert_closed, role_entity_gae
...
_last_critic_roles: tuple[str, ...] | None = None
...
def _road_focused_enabled() -> bool:
    return os.environ.get("COV2X_ROAD_FOCUSED_EXTENSION_V1", "0") == "1"
...
def _runtime_candidate_id() -> str:
    if _road_focused_enabled():
        return ROAD_FOCUSED_CANDIDATE_ID
...
def _runtime_checkpoint_format_version() -> int:
    if _road_focused_enabled():
        return ROAD_FOCUSED_CHECKPOINT_FORMAT_VERSION
    return (
        TEMPORARY_SPEED_CAP_CHECKPOINT_FORMAT_VERSION
        if _temporary_speed_cap_enabled()
...
def _runtime_actor_update_schedule_id() -> str:
    if _road_focused_enabled():
        ...
        if schedule_id != ROAD_FOCUSED_ACTOR_UPDATE_SCHEDULE_ID:
            raise ValueError("Road-focused actor update schedule mismatch")
        return schedule_id
...
def _runtime_frozen_actor_roles() -> tuple[str, ...]:
    if _road_focused_enabled():
        return ROAD_FOCUSED_FROZEN_ACTOR_ROLES
    return FROZEN_ACTOR_ROLES


def _runtime_critic_lineage() -> str:
    if _road_focused_enabled():
        return ROAD_FOCUSED_CRITIC_LINEAGE
    return LOCAL_CREDIT_CRITIC_LINEAGE if _local_credit_enabled() else CRITIC_LINEAGE


def _runtime_critic_reward_semantics() -> str | None:
    if _road_focused_enabled():
        return ROAD_FOCUSED_REWARD_SEMANTICS
    return LOCAL_CREDIT_REWARD_SEMANTICS if _local_credit_enabled() else None
...
def reset_untrained_state() -> None:
    ...
    global _last_trainable_actor_roles, _last_critic_roles
...
def _migrate_road_focused_parent(
    state: Mapping[str, Any], actual_sha256: str
) -> None:
    """Copy all update-24 actors and reset Road-training value state."""

    global _policy_generation, _critic_updates, _parent_provenance_verified
    global _last_trainable_actor_roles, _last_critic_roles
    global _critic, _optimizers, _value_normalizer
    if not _road_focused_enabled():
        raise ValueError("Road-focused parent requires the Road extension profile")
    expected = {
        "candidate_id": ROAD_FOCUSED_PARENT_CANDIDATE_ID,
        "format_version": ROAD_FOCUSED_PARENT_CHECKPOINT_FORMAT_VERSION,
        "policy_generation": ROAD_FOCUSED_PARENT_CHECKPOINT_GENERATION,
        "vehicle_action_semantics": TEMPORARY_SPEED_CAP_ACTION_SEMANTICS,
        "actor_update_schedule_id": (
            TEMPORARY_SPEED_CAP_THREE_SCOPE_ACTOR_UPDATE_SCHEDULE_ID
        ),
    }
    if actual_sha256 != ROAD_FOCUSED_PARENT_CHECKPOINT_SHA256:
        raise ValueError("Road-focused direct parent SHA-256 mismatch")
    for field, value in expected.items():
        if state.get(field) != value:
            raise ValueError(f"Road-focused direct parent {field} mismatch")
    _validate_common_checkpoint(state)
...
def _validate_road_focused_training_state(
    *,
    generation: int,
    critic_updates: int,
    ...
    if generation == 0 and critic_updates == 0:
        expected = ((), ())
    elif generation == 0 and critic_updates == 1:
        expected = ((), ("road",))
    elif 1 <= generation <= ROAD_FOCUSED_ACTOR_UPDATES:
        if critic_updates != generation + 1:
            raise ValueError("Road-focused critic update count violates schedule")
        expected = (("road",), ("road",))
    else:
        raise ValueError("Road-focused generation/critic update state is invalid")
    if (trainable_roles, critic_roles) != expected:
        raise ValueError("Road-focused actor/critic role state violates schedule")


def _load_road_focused_checkpoint(state: Mapping[str, Any]) -> None:
    ...
    global _last_critic_roles, _parent_provenance_verified
    expected = {
        "candidate_id": ROAD_FOCUSED_CANDIDATE_ID,
        "format_version": ROAD_FOCUSED_CHECKPOINT_FORMAT_VERSION,
        "vehicle_action_semantics": TEMPORARY_SPEED_CAP_ACTION_SEMANTICS,
        "parent_candidate_id": ROAD_FOCUSED_PARENT_CANDIDATE_ID,
        "parent_checkpoint_format_version": (
            ROAD_FOCUSED_PARENT_CHECKPOINT_FORMAT_VERSION
        ),
        "parent_checkpoint_generation": ROAD_FOCUSED_PARENT_CHECKPOINT_GENERATION,
        "parent_checkpoint_sha256": ROAD_FOCUSED_PARENT_CHECKPOINT_SHA256,
        "critic_lineage": ROAD_FOCUSED_CRITIC_LINEAGE,
        "critic_reward_semantics": ROAD_FOCUSED_REWARD_SEMANTICS,
        "actor_update_schedule_id": ROAD_FOCUSED_ACTOR_UPDATE_SCHEDULE_ID,
        "frozen_actor_roles": list(ROAD_FOCUSED_FROZEN_ACTOR_ROLES),
    }
    for field, value in expected.items():
        if state.get(field) != value:
            raise ValueError(f"Road-focused checkpoint {field} mismatch")
    expected_config_hash = os.environ.get("COV2X_ROAD_FOCUSED_CONFIG_HASH")
    if (
        expected_config_hash
        and state.get("road_focused_config_hash") != expected_config_hash
    ):
        raise ValueError("Road-focused checkpoint config hash mismatch")
    _validate_common_checkpoint(state)
    assert _cloud_actor is not None
    assert _road_actor is not None
    assert _vehicle_actor is not None
    assert isinstance(_critic, ConditionedCentralizedCritic)
    components = {
    ...
    if state.get("component_schema") != expected_schema:
        raise ValueError("Road-focused component schema fingerprint mismatch")
    ...
    if set(state.get("optimizers", {})) != {"cloud", "road", "vehicle", "critic"}:
        raise ValueError("Road-focused optimizer role set mismatch")
    ...
    if set(rng_state or {}) != {"python", "numpy", "torch_cpu"}:
        raise ValueError("Road-focused checkpoint RNG state contract mismatch")
    random.setstate(rng_state["python"])
    np.random.set_state(rng_state["numpy"])
    torch.set_rng_state(rng_state["torch_cpu"])
    saved_trainable = tuple(
        str(role) for role in state.get("trainable_actor_roles_at_save", ())
    )
    saved_critic_roles = tuple(
        str(role) for role in state.get("critic_training_roles_at_save", ())
    )
    critic_updates = int(state.get("critic_updates", -1))
    _validate_road_focused_training_state(
        generation=generation,
        critic_updates=critic_updates,
        trainable_roles=saved_trainable,
        critic_roles=saved_critic_roles,
    )
    _policy_generation = generation
    _critic_updates = critic_updates
    ...
    _last_trainable_actor_roles = saved_trainable
    _last_critic_roles = saved_critic_roles
    _parent_provenance_verified = True
...
def _load_local_credit_checkpoint(state: Mapping[str, Any]) -> None:
    global _policy_generation, _critic_updates, _initialization_seed
    global _initialization_seed_role, _last_trainable_actor_roles
    global _parent_provenance_verified, _last_critic_roles
    ...
    _last_critic_roles = tuple(
        str(role)
        for role in state.get("critic_training_roles_at_save", ("vehicle",))
    )
    if set(_last_critic_roles) - {"vehicle"}:
        raise ValueError("Vehicle-local checkpoint restored non-Vehicle critic role")
    _parent_provenance_verified = True
...
def _maybe_load_checkpoint() -> None:
    ...
    road_parent_path = os.environ.get("COV2X_ROAD_FOCUSED_PARENT_MODEL_PATH")
    provided = [
        value
        for value in (model_path, parent_path, local_parent_path, road_parent_path)
        if value
    ]
    if len(provided) > 1:
    ...
    if road_parent_path:
        ...
        _migrate_road_focused_parent(state, actual_sha)
    elif local_parent_path:
        actual_sha = sha256(target.read_bytes()).hexdigest()
        _migrate_g30_local_credit(state, actual_sha)
    elif parent_path:
        actual_sha = sha256(target.read_bytes()).hexdigest()
        _migrate_parent_checkpoint(state, actual_sha)
    elif state.get("candidate_id") == ROAD_FOCUSED_CANDIDATE_ID:
        os.environ["COV2X_ROAD_FOCUSED_EXTENSION_V1"] = "1"
        _load_road_focused_checkpoint(state)
...
def restore_full_checkpoint(path: str) -> None:
    ...
    state = torch.load(target, map_location="cpu", weights_only=False)
    road_focused = state.get("candidate_id") == ROAD_FOCUSED_CANDIDATE_ID
    ...
    if road_focused:
        ...
        os.environ.pop("COV2X_ROAD_FOCUSED_EXTENSION_V1", None)
...
def initialize(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _ledger, _transport, _leaders = VehicleLifecycleLedger(), IdealPhasedTransport(), StickyLeadCAV()
    _rollout = MVPRollout(
        _episode_id,
        _period,
        _seed,
    ...
    _last_total = 0.0
    _initialized = True
    return {"protocol_version": "2.0", "candidate_id": _runtime_candidate_id(), "episode_id": _episode_id, "policy_generation": _policy_generation, "ready": True}
...
def _road_actions(payload: Mapping[str, Any], snapshot_id: str, sim_time: float, state: np.ndarray) -> tuple[dict[str, dict[str, int]], list[SMDPTransition]]:
    ...
    for tls_id, order in _phase_orders.items():
        ...
        transitions.append(SMDPTransition("road", snapshot_id, {"q": q_values, "road": road_obs, "cloud": cloud, "phase": phase_features, "movement": movement_features, "mask": mask, "authority_gain": gain, "critic_context": value_context.copy()}, action_index, float(logprob.cpu().item()), _critic_value(state, role="road", intersection_id=tls_id, critic_context=value_context), 0.0, 5.0, policy_generation=_policy_generation, causal_parents=parents, executed_action=action_index, entity_id=tls_id, decision_time_s=sim_time, global_state=state.copy()))
    return actions, transitions
...
def step(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"protocol_version": "2.0", "candidate_id": _runtime_candidate_id(), "episode_id": _episode_id, "step_id": step_id, "actions": {"signals": signal_actions, "vehicles": vehicle_actions}, "diagnostics": response_diagnostics}
...
def train_on_rollouts(
    ...
    *,
    ...
    critic_roles: Sequence[str] | None = None,
) -> dict[str, Any] | None:
    ...
    if critic_roles is None:
        requested_critic_roles = (
            ("road",)
            if _road_focused_enabled()
            else (("vehicle",) if _local_credit_enabled() else ())
        )
    else:
        requested_critic_roles = tuple(
            dict.fromkeys(str(role) for role in critic_roles)
        )
    unknown_critic_roles = set(requested_critic_roles) - {
        "cloud", "road", "vehicle", "value"
    }
    if unknown_critic_roles:
        raise ValueError(
            f"unknown critic roles: {sorted(unknown_critic_roles)}"
        )
    if _road_focused_enabled() and (
        requested_roles != ("road",)
        or requested_critic_roles != ("road",)
    ):
        raise ValueError("Road-focused updates require Road actor and Road critic only")
    if (
        _local_credit_enabled()
        and not _road_focused_enabled()
        and requested_critic_roles != ("vehicle",)
    ):
        raise ValueError("Vehicle-local updates require Vehicle critic transitions")
    ...
    critic_transitions = (
        [item for item in transitions if item.role in requested_critic_roles]
        if requested_critic_roles
        else transitions
    )
    if requested_critic_roles and not critic_transitions:
        raise ValueError("requested critic roles have no transitions")
    diagnostics["critic"] = _critic_update(critic_transitions)
    _last_trainable_actor_roles = requested_roles
    _last_critic_roles = requested_critic_roles
    _critic_updates += 1
    ...
    return {
        ...
        "critic_roles": list(requested_critic_roles),
...
def train_road_critic_on_rollouts(
    rollouts: list[MVPRollout | None],
) -> dict[str, Any] | None:
    """One frozen-actor burn-in update on Road global-reward transitions."""
    ...
    if not valid:
        return None
    if not _road_focused_enabled() or _mode != "train":
        raise ValueError("Road critic burn-in requires Road-focused TRAIN mode")
    if any(item.policy_generation != _policy_generation for item in valid):
        raise ValueError("Road critic burn-in policy generation mismatch")
    expected_gains = {"road": 1.0, "cloud": 1.0, "vehicle": 1.0}
    if any(item.authority_gains != expected_gains for item in valid):
        raise ValueError("Road critic burn-in requires full authority")
    _authority_gains = dict(valid[0].authority_gains)
    preflight = preflight_actor_samples(valid, required_roles=("road",))
    transitions = [
        transition
        for item in valid
        for transition in item.transitions
        if transition.role == "road"
    ]
    if not transitions:
        raise ValueError("Road critic burn-in has no Road transitions")
    diagnostics = _critic_update(transitions)
    _critic_updates += 1
    _last_trainable_actor_roles = ()
    _last_critic_roles = ("road",)
    return {
        "candidate_id": _runtime_candidate_id(),
        "policy_generation": _policy_generation,
        "critic_updates": _critic_updates,
...
def save_checkpoint(path: str) -> str:
    ...
    road_focused = _road_focused_enabled()
    if _local_credit_enabled():
        if not isinstance(_critic, ConditionedCentralizedCritic):
            raise RuntimeError("local-credit checkpoint critic architecture mismatch")
        allowed_actor_roles = {"road"} if road_focused else {"vehicle"}
        allowed_critic_roles = {"road"} if road_focused else {"vehicle"}
        if set(_last_trainable_actor_roles or ()) - allowed_actor_roles:
            raise RuntimeError("checkpoint trainable actor role violates schedule")
        if set(_last_critic_roles or ()) - allowed_critic_roles:
            raise RuntimeError("checkpoint critic role violates schedule")
        if _policy_generation == 0 and not road_focused:
            target_bias = float(
    ...
    critic_roles_at_save = tuple(_last_critic_roles or ())
    if road_focused:
        _validate_road_focused_training_state(
            generation=_policy_generation,
            critic_updates=_critic_updates,
            trainable_roles=trainable_at_save,
            critic_roles=critic_roles_at_save,
        )
    frozen_actor_roles = _runtime_frozen_actor_roles()
    current_frozen = [
        role
        for role in ("cloud", "road", "vehicle")
    ...
    state = {
        ...
        "critic_lineage": _runtime_critic_lineage(),
        "critic_reward_semantics": _runtime_critic_reward_semantics(),
        "local_credit_reward_semantics": (
        ...
        "road_focused_config_hash": (
            os.environ.get("COV2X_ROAD_FOCUSED_CONFIG_HASH")
            if road_focused
            else None
        ),
        "initial_deterministic_vehicle_mean": (
            None
            if road_focused
            else (_runtime_initial_vehicle_mean() if local_credit else 0.0)
        ),
        "popart": None,
        "frozen_actor_roles": list(frozen_actor_roles),
        ...
        "critic_training_roles_at_save": list(critic_roles_at_save),
        "actor_update_schedule_id": (
            _runtime_actor_update_schedule_id()
            if local_credit
