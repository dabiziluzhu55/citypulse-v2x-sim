...
def edge_lane_indices(edge_id: str) -> tuple[int, ...]:
    """Sorted lane indices of an ordinary edge from the init-time static table.

    Returns an empty tuple when the edge is unknown or ``edge_lanes`` was not
    provided at initialization. Used by the CoV2X vehicle observation builder
    to validate Protocol 2.0 ``target_lane_index`` commands.
    """
    indices: list[int] = []
    for lane_id in _edge_to_lanes.get(str(edge_id), []):
...
def green_remaining(tls_id: str, intersection_obs: dict) -> float:
    """Remaining green seconds of the current phase; ``-1`` when not GREEN.

    Mirrors the controller's runtime helper without importing the controller
    (which would pull in the full policy stack). Phase durations come from the
    init-time static table, matching how the lane-state builder consumes them.
    """
    if not isinstance(intersection_obs, dict):
        return -1.0
    if str(intersection_obs.get("stage", "")) != "GREEN":
        return -1.0
    current_phase = int(intersection_obs.get("current_phase", 0))
    total = _phase_durations.get(str(tls_id), {}).get(current_phase, 0.0)
    return max(0.0, total - float(intersection_obs.get("stage_elapsed", 0.0)))


def phase_movements(tls_id: str) -> dict[int, frozenset[str]]:
    """Return ``{phase_id: frozenset(movement)}`` for a controlled intersection."""
    result: dict[int, frozenset[str]] = {}
    for pid, movements in _phase_metadata.get(str(tls_id), {}).items():
        result[int(pid)] = frozenset(movements)
    return result


def lane_movements(lane_id: str) -> tuple[str, ...]:
    """Movement strings of a lane's outgoing connections, in metadata order."""
    return tuple(
        str(conn.get("movement", ""))
        for conn in _lane_connections.get(str(lane_id), [])
        if conn.get("movement")
    )


def lane_allowed_types(edge_id: str, lane_index: int) -> frozenset[str]:
    """Vehicle type IDs allowed on a lane; empty means no type is allowed."""
    return frozenset(
        _lane_permissions.get(str(edge_id), {}).get(int(lane_index), set())
    )
