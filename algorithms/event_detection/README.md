...
| `localized_blockage` | 局部车道受阻 | 绿灯期间目标车道释放异常，但相邻车道仍较正常 |
...
规则首先判断车道是否出现持续异常。红灯及绿灯刚开始的短暂启动损失不作为异常证据；只有在车道拥有通行权、异常持续多个采样点且满足速度、停车、等待和占有率等条件时，才确认异常。恢复正常后保留短暂解除延迟，避免告警闪烁。

确认异常后，规则诊断交通状态：

| 输出 | 判断依据 |
| --- | --- |
| `localized_blockage` | 目标车道在具备通行权时持续受阻，而相邻可比较车道仍能正常释放 |
