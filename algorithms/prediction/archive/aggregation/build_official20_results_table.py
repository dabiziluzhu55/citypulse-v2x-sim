"""Build a single comparable 60-second results table for official-20 prediction."""
...
def build_rows(
    *, baseline_path: Path, xgb_path: Path, stgcn_path: Path, horizon_seconds: float = 60.0
) -> list[dict[str, object]]:
    """Load matching metrics and return rows in presentation order.

    The model artefacts are direct 60-second forecasts. Baseline rows are
    filtered to the same horizon so every value in the result table measures
    ...
    missing = [f"{split}/{model}" for split in SPLITS for model in MODELS if (split, model) not in indexed]
    if missing:
...
def write_results(rows: list[dict[str, object]], *, csv_path: Path, markdown_path: Path) -> None:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Build the official-20 comparable 60-second results table.")
