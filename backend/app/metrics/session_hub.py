...
from .powertrain import load_powertrain_by_type
...
class SessionMetricsHub:
    ...
    def start_session(self, session_id: str, control_mode: str) -> None:
        with self._lock:
            ...
            powertrain, warnings = load_powertrain_by_type(
                session_dir=session_dir,
                traffic_manifest_path=self._traffic_manifest_path,
            )
            if powertrain:
                collector.set_powertrain_by_type(powertrain)
            else:
    ...
    def _load_persisted(self, session_id: str) -> EvalResult | None:
        ...
        return EvalResult(
            ...
            fuel_intensity_L_per_100km=payload.get("fuel_consumption"),
