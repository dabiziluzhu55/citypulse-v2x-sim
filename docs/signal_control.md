...
不能再与进口列表同时使用。
若拓扑把辅助大角度 `R` 连接显式映射为右转，且它与同一进口的另一个让行 movement 被
SUMO foe 矩阵判定为 mutual `g/g`，该辅助右转会仅在冲突相位保持红灯，并在前一相位执行
`g -> y -> r` 过渡。普通小写 `r` 右转不参与这项安全收缩，即使同一进口存在多条右转
connection，也始终保持让行绿。这避免 `demo_9` 特例误伤普通多车道右转。
...
`demo_9` 东北进口右转只在官方相位 3 与东北左转一起使用受保护绿，其他相位保持红灯。
`demo_9` 东进口 `-56619` 的左转 link `44` 与第二条右转 link `46` 属于同进口的 mutual
`g/g`；link `46` 因此只在官方相位 2 保持红灯，在相位 1 黄灯阶段完成转红，并在相位 3
恢复让行绿。普通右转 link `45` 始终使用 `g`。
