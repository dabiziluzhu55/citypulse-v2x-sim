...
const helperPath = path.resolve(scriptsDirectory, 'generate-edge-topology-map.py')
const argumentsForHelper = [
  ...
  path.resolve(frontendDirectory, 'public/intersections/v3/edge-to-topology-segment.json'),
]
...
throw new Error(`Edge topology map generation failed:\n${failures.join('\n')}`)
