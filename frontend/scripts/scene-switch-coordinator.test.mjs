...
test('prepares a vehicle stage before camera flight and commits before road activation', () => {
  ...
  assert.match(vehicleRendererSource, /presentImmediate\(result\.samples, 'view_reactivated'\)/)
  assert.match(vehicleRendererSource, /resetTwinPresentation\('view_reactivated_first_ready_frame'\)/)
  assert.match(threeMapSource, /initial scene committed with deferred vehicle hydration/)
  assert.match(threeMapSource, /vehicleRenderer\?\.bootstrapViewport\(intersectionId, headingResolver, poseResolver\)/)
