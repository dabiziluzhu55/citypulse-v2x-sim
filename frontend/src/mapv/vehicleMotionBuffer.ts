...
export function interpolateVehicleTwinSample(
  ...
  return {
    ...
    dir: left.dir + shortestAngleDelta(left.dir, right.dir) * amount,
    time: 0,
  }
}
...
export class VehicleMotionBuffer {
  ...
  private bufferSeconds = MIN_BUFFER_SECONDS
  ...
  sample(wallTimeMs: number): VehicleTwinSample[] | null {
    ...
    if (this.renderElapsedSeconds == null) {
      ...
      this.renderElapsedSeconds = Math.min(
        maximumTarget,
        this.renderElapsedSeconds + wallDeltaSeconds * this.sourceRate,
      )
    }
  ...
  reset(): void {
    ...
    this.lastOutputWallTimeMs = null
    this.sourceRate = 1
    this.bufferSeconds = MIN_BUFFER_SECONDS
  }
  ...
  private updateTiming(
    ...
    if (wallDeltaMs <= 0 || wallDeltaMs > 2_000 || simulationDelta <= 0) {
      this.arrivalIntervalsMs = []
      return
    }
    ...
    const jitterIntervalSeconds = percentile(this.arrivalIntervalsMs, 0.95) / 1_000
    const requestedBuffer = jitterIntervalSeconds * this.sourceRate * BUFFER_INTERVAL_MULTIPLIER
    this.bufferSeconds += (
      clamp(requestedBuffer, MIN_BUFFER_SECONDS, MAX_BUFFER_SECONDS) - this.bufferSeconds
    ) * 0.2
  }

  private resolveSourceStepSeconds(): number {
    if (this.frames.length < 2) return MIN_BUFFER_SECONDS / 2
