...
"$PYTHON" -m algorithms.prediction.build_intersection20_adjacency --tls-manifest "$SNAPSHOT_DIR/data/maps/sumo/generated/manifests/tls_manifest.json" --intersections "$SNAPSHOT_DIR/data/maps/sumo/TotalMap_20.intersections.json" --output "$OUT/adjacency.npz" --neighbors 3 > "$OUT/adjacency.log"
tail -n +2 "$EXPERIMENT_DIR/collection_manifest_prediction_v1.tsv" | while IFS=$'\t' read -r id split period scale seed duration interval; do
  ...
  "$PYTHON" -m algorithms.prediction.aggregate_intersection20_snapshots --input "$EXPERIMENT_DIR/raw/${id}_5s_lanes.csv" --output "$out" --tls-manifest "$SNAPSHOT_DIR/data/maps/sumo/generated/manifests/tls_manifest.json" >> "$OUT/aggregate.log" 2>&1
