...
from traffic_control.cov2x.collab.joint_rewards import (
    TEAM_WEIGHT_DEFAULT,
    TrafficSnapshot,
    cloud_local_reward,
    signal_local_reward,
...
from traffic_control.cov2x.vehicle.observations import build_vehicle_observations
from traffic_control.cov2x.vehicle.rewards import vehicle_reward, vehicle_reward_components
from traffic_control.cov2x.vehicle.rollout import (
    Rollout,
    RolloutStep,
    compute_gae,
    episode_summary,
...
from traffic_control.cov2x.road.signal import fixed_actions, max_pressure_actions
...
def _lazy_lane_state() -> Any:
    ...
    if _lane_state is None:
        from traffic_control.cov2x.road import lane_state
...
def _lazy_policy() -> Any:
    global _policy
    if _policy is None:
        from traffic_control.cov2x.vehicle.policy import VehiclePPOAgent, VehiclePolicyConfig
...
def _lazy_joint_policy() -> Any:
    ...
    if _joint_policy is None:
        from traffic_control.cov2x.collab.joint_policy import (
            JointPPOAgent,
            JointPolicyConfig,
        )
...
def _sample_cloud(
    ...
    *,
    ...
    from traffic_control.cov2x.cloud.observations import build_cloud_observation
...
def step(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if _mode == "train":
        ...
        if _vehicle_mode == "off":
            ...
            from traffic_control.cov2x.vehicle.rule import rule_vehicle_actions
