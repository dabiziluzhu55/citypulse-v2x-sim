...
from .road_hooks import FrozenRoadHooks
from .transport import MessageBus, PermissionBook, digest, jsonable
from .rollout import EpisodeBuffer
...
def _source_version():
    return hashlib.sha256("".join(p.name + hashlib.sha256(p.read_bytes()).hexdigest()
                                  for p in sorted(HERE.glob("*.py"))
                                  if not p.name.startswith('.')).encode()).hexdigest()
