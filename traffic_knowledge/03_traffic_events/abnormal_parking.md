...
LLM 可将其标为“疑似停止车辆型阻塞”，建议核验车辆轨迹和路侧感知；控制策略可减少向受阻方向放行并观察替代 movement。实体处置和违法认定超出本系统范围。

## LLM 判断指标

车辆速度/位置持续性、是否计划停车、车道速度、停止数、等待增量、占用率、相邻车道对照、信号阶段与下游状态。若无法获得车辆级持续轨迹，应降级为 `lane_blocked` 或“原因未知的局部阻塞”，不要强判异常停车。

## 当前项目能力边界

事件检测语义能把 `stopped_vehicle` 等外部标签归一到 `lane_blocked`，但这不等于产品已实现异常停车识别。仿真扰动 API 也没有独立异常停车请求。LLM 可以解释或提出待核验假设，不可声称系统已直接检测并确认异常停车。
