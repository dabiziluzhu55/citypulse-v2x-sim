...
车辆类型阈值来自 `data/maps/sumo/vehicle_profiles.json`：不同类型可不同。这些是项目仿真配置，不是现实事故判定标准。
...
项目正式 `EvalResult` 尚无碰撞数、TTC、PET、DRAC、行人/非机动车冲突或实测事故数据。SUMO 可通过 SSM device 输出 TTC、DRAC、PET 等安全替代指标，但当前 `traffic_eval` 没有接入这些字段，LLM 不得把它们说成已实现。
...
3. citypulse-v2x-sim
   ...
   - branch: main
   - file: simulation/sumo/vehicle.py; data/maps/sumo/vehicle_profiles.json; traffic_eval/collector.py; traffic_eval/models.py
