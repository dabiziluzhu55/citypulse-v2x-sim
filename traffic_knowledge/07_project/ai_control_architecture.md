...
**【项目事实】** CityPulse-Qwen 面向用户在前端表单中提交的扰动事件（施工占道、道路限速、大型活动开场/散场、交通事故），融合实时交通状态、事件信息、短时预测、道路拓扑、当前信号状态和系统实际控制能力，对扰动影响区域生成局部多路口协同信号管控方案。AI Control **不把 RAG 文本写入 Observation V2**。
...
**【项目事实】** 产品 `control_mode` 现为 `fixed`、`sotl`、`max_pressure`、`ippo`、`mappo`、`cov2x`。用户在前端选择其中之一后，该算法独立运行，用于相同场景下的公平对比。CityPulse-Qwen **不是** 新的 `control_mode`，也 **不负责** 选择哪个算法。

**【项目事实】** 仅当用户创建扰动，并主动选择“启用 AI 管控”时：
...
**【项目事实】** AI takeover 的触发条件为：启动时至多一个扰动 `ai_control_enabled=true`，且该注入事件处于可接管状态。检测卡片可用于提示，不得自动接管。
